...
while IFS='=' read -r key value; do
  [[ -z "$key" || "$key" == \#* ]] && continue
  case "$key" in
    AI_PROVIDER|NODE_ENV|NODE_COMMAND|CODEX_COMMAND|CODEX_MODEL|CODEX_TIMEOUT_MS|CODEX_WORKDIR|DB_HOST|DB_PORT|DB_NAME|DB_USER|DATABASE_SSL|NODE_EXTRA_CA_CERTS|WORKER_ID|WORKER_CONCURRENCY|WORKER_HEARTBEAT_MS|JOB_POLL_MS|JOB_LEASE_MS|JOB_MAX_ATTEMPTS|BROWSER_TIMEOUT_MS|MAX_AUDIT_PAGES|MAX_NETWORK_HOSTS|MAX_SCREENSHOT_BYTES|KEYCHAIN_ACCOUNT|KEYCHAIN_SERVICE)
      export "$key=$value"
      ;;
    *)
