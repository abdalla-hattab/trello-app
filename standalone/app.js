    ...
    boards.forEach(board => {
        if (board.lists && board.connections) {
            ...
            board.lists.forEach(l => {
                const isTracker = l.trelloListId || l.trelloTasksListId || l.trelloTasks2ListId || l.pipedriveStageId || l.trackerType;
                if (isTracker) {
...
window.handleToggleReorder = function(e, listId, edge, targetType) {
    ...
    const map = { 'ch': 'clientHappiness', 'ms': 'moneySmelling', 'nc': 'newClients', 'pd': 'pipedrive', 'trello': 'trello', 'trello-speech': 'trelloSpeech', 'trello3': 'trello3', 'ads': 'ads' };
    ...
    let curOrder = list.edgeOrder[edge] || ['clientHappiness', 'moneySmelling', 'newClients', 'pipedrive', 'trello', 'trello3', 'trelloSpeech', 'ads', 'ads2'];
...
window.openTrelloCardDetailsModal = async function(cardId, listId) {
    ...
    const isAdsTracker = targetLocalList && (targetLocalList.trackerType === 'ads' || targetLocalList.trackerType === 'ads2');
        ...
        if (targetLocalCard && targetLocalCard.isTrelloDeleted) {
            ...
            if (reactivateBtn) {
                reactivateBtn.onclick = async () => {
                    ...
                    try {
                        const targetListId = targetLocalCard.trelloListId || targetLocalCard.trelloTasksListId || targetLocalList.trelloListId || targetLocalList.trelloTasksListId || targetLocalList.trelloTasks2ListId;
...
if (generateTrelloTasksBtn) {
    if(generateTrelloTasksBtn) generateTrelloTasksBtn.onclick = () => {
        ...
        if (window.pendingTrelloTasksIndex === 2) {
            pendingSourceList.trelloTasks2ListId = listSelect.value;
            pendingSourceList.trelloTasks2BoardId = boardSelect.value;
        } else {
...
function updateAllTrackersSummaries(activeBoard) {
    ...
    activeBoard.lists.forEach(list => {
        ...
        const isTrelloTracker = (list.trelloListId || list.trelloBoardId || ( (list.trelloTasksListId || list.trelloTasks2ListId) && (!list.title || list.title.toLowerCase() !== 'me') )) && list.trackerType !== 'ads' && list.trackerType !== 'ads2' && !list.isClientHappiness && !list.isMoneySmelling;
        ...
        if (hasOutgoing || isAdsTracker || isTrelloTracker) {
            ...
            if (allDescendants.size > 0) {
                ...
                if (pdLeafNodes.length > 0) {
                ...
                } else if (list.trackerType === 'ads' || list.trackerType === 'ads2' || list.trackerType === 'trelloSpeech' || list.trackerType === 'trello3' || list.trelloListId || list.trelloBoardId || ( (list.trelloTasksListId || list.trelloTasks2ListId) && (!list.title || list.title.toLowerCase() !== 'me') )) {
                    let hasTrello = false;
                    let hasAds = false;
                    let hasAds2 = false;
                    let hasTs = false;
                    ...
                    let t3Cards = 0; let t3Col = { green: 0, yellow: 0, orange: 0, red: 0, default: 0 };
...
function renderKanbanApp(activeBoard) {
        ...
        activeBoard.connections.forEach(conn => {
            ...
            if (toggleEl) {
                ...
                activeBoard.connections.forEach(c => {
                    if (c.source === conn.source && c.sourcePort === sourceEdge) {
                        ...
                        if (targList) {
                            ...
                            if (targList.trackerType === 'trello3') hasTrello3 = true;
                            if ((targList.trelloTasksListId || targList.trelloTasks2ListId || targList.trelloBoardId || targList.trelloListId) && targList.trackerType !== 'ads' && targList.trackerType !== 'ads2' && targList.trackerType !== 'trelloSpeech' && targList.trackerType !== 'trello3') hasTrello = true;
                ...
                if (targetList) {
                    ...
                    else if (targetList.trackerType === 'trello3') myType = 'trello3';
                }
                ...
                if (hasTrello3) activeTypes.push('trello3');
                if (hasAds) activeTypes.push('ads');
                if (hasAds2) activeTypes.push('ads2');
                
                const sList = activeBoard.lists.find(l => l.id === conn.source);
                const curOrder = sList && sList.edgeOrder && sList.edgeOrder[sourceEdge] ? sList.edgeOrder[sourceEdge] : ['clientHappiness', 'moneySmelling', 'newClients', 'pipedrive', 'trello', 'trello3', 'trelloSpeech', 'ads', 'ads2'];
            ...
            if (c.source === sourceId && (edge === null || c.sourcePort === edge)) {
                ...
                if (tType) {
                    ...
                    if (tl) {
                        ...
                        if (tType === 'trello3' && tl.trackerType === 'trello3') matches = true;
                        if (tType === 'trello' && (tl.trelloTasksListId || tl.trelloTasks2ListId || tl.trelloBoardId || tl.trelloListId) && tl.trackerType !== 'ads' && tl.trackerType !== 'ads2' && tl.trackerType !== 'trelloSpeech' && tl.trackerType !== 'trello3') matches = true;
    ...
    if (activeBoard.sentimentFilters) {
        Object.keys(activeBoard.sentimentFilters).forEach(key => {
            ...
            allD.forEach(childId => {
                ...
                if (cl && cl.id !== pId) {
                    if (pType === 'trello' && (cl.trelloTasksListId || cl.trelloTasks2ListId || cl.trelloBoardId || cl.trelloListId) && cl.trackerType !== 'ads' && cl.trackerType !== 'ads2') {
    ...
    activeBoard.lists.forEach((list, listIndex) => {
        ...
        if (!list.trelloTasksListId && !list.trelloTasks2ListId && (list.trelloBoardId || list.trelloListId) && list.trackerType !== 'ads' && list.trackerType !== 'ads2' && list.cards.length === 0 && !hiddenListIds.has(list.id) && !isMeList) {
            isFilteredOut = true;
        }
    ...
    const listContainer = document.createElement('div');
        ...
        if (list.pipedriveStageId || list.trackerType === 'ads' || list.trackerType === 'ads2') listContainer.classList.add('auto-height-list');
        ...
        listContainer.ondrop = (e) => {
            ...
            if (transferTypeObj) {
                ...
                if (sourceId && sourceId !== list.id) {
                    const mappedType = trackerType === 'pd' ? 'pipedrive' : (trackerType === 'ch' ? 'clientHappiness' : (trackerType === 'ms' ? 'moneySmelling' : (trackerType === 'nc' ? 'newClients' : (trackerType === 'trello3' ? 'trello3' : (trackerType === 'trello-speech' ? 'trelloSpeech' : (trackerType === 'ads2' ? 'ads2' : trackerType))))));
                    const formatNameMap = { 'pipedrive': 'Pipedrive', 'clientHappiness': 'Client Happiness', 'moneySmelling': 'Money Smelling', 'newClients': 'New Clients', 'trello': 'Trello', 'ads': 'Ads', 'ads2': 'Ads Tracker 2', 'trelloSpeech': 'Trello Tracker 2', 'trello3': 'Trello Tracker 3' };
                    ...
                    const checkMatch = (t) => {
                        ...
                        if (mappedType === 'trello3') return t.trackerType === 'trello3';
                        if (mappedType === 'trello') return (t.trelloListId || t.trelloTasksListId || t.trelloTasks2ListId) && t.trackerType !== 'ads' && t.trackerType !== 'ads2' && t.trackerType !== 'trelloSpeech' && t.trackerType !== 'trello3' && !t.isClientHappiness && !t.isMoneySmelling && !t.isNewClients;
                        return false;
                    };
                        ...
                        if (transferred) {
                            ...
                            if (srcList) {
                                if (mappedType === 'pipedrive') {
                                    ...
                                    if (srcList.trelloTasks2ListId) { list.trelloTasks2ListId = srcList.trelloTasks2ListId; delete srcList.trelloTasks2ListId; }
                                    ...
                                    if (srcList.trackerType === 'ads' || srcList.trackerType === 'ads2') delete srcList.trackerType;
        ...
        header.onmousedown = (e) => {
            ...
            const getGroupType = (l) => {
                ...
                if (l.trackerType === 'trello3') return 'trello3';
                if (l.trackerType === 'ads') return 'ads';
                if (l.trackerType === 'ads2') return 'ads2';
                if (l.trelloTasksListId || l.trelloTasks2ListId) return null; // Explicitly decouple Trello Tasks from cluster grouping
            ...
            const onMouseUp = () => {
                ...
                const groupType = getGroupType(list);
                if ((groupType === 'trello' || groupType === 'trelloSpeech' || groupType === 'trello3') && !shiftUsedDuringDrag) {
                    ...
                    if (motherList) {
                        ...
                        if (groupType === 'trello3') {
                            if (motherList.trello3OffsetX === undefined) motherList.trello3OffsetX = 800;
                            if (motherList.trello3SpacingY === undefined) motherList.trello3SpacingY = 60;
                            motherList.trello3OffsetX += totalDx;
                            motherList.trello3SpacingY -= totalDy;
                        } else if (groupType === 'trelloSpeech') {
        ...
        if ((list.trelloListId || list.trelloTasksListId || list.trelloTasks2ListId || list.trackerType === 'trello') && list.trackerType !== 'ads' && list.trackerType !== 'ads2') {
        ...
        if (activeBoard.trelloBoardId && !list.trelloListId) {
            const opt = document.createElement('div');
            opt.className = 'list-option-item';
            opt.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span>Trello Tracker 1</span>`;
            opt.onclick = (e) => {
                e.stopPropagation();
                openTrelloMappingGenerator(list);
                optionsMenu.style.display = 'none';
            };
        ...
        if (activeBoard.trelloBoardId && (list.trackerType === 'ads2' || !list.trelloListId)) {
            const isUnlink = list.trelloListId && list.trackerType === 'ads2';
            const icon = isUnlink ? `<line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line>` : `<polyline points="23 6 13.5 15.5 8.5 10.5 1 18"></polyline><polyline points="17 6 23 6 23 12"></polyline>`;
            const text = isUnlink ? 'Unlink Ads Tracker' : 'Ads Tracker 2';
            ...
            if(opt) opt.onclick = (e) => {
                e.stopPropagation();
                if (list.trelloListId && list.trackerType === 'ads2') {
                    if(confirm("Unlink this Ads tracker layout?")) {
                        list.trelloListId = null;
                        list.trackerType = null;
                        list.cards = list.cards.filter(c => !c.isTrello);
                ...
                } else openTrelloMappingGenerator(list, 'ads2');
                optionsMenu.style.display = 'none';
            };
            optionsMenu.appendChild(opt);
        }
        ...
        if (activeBoard.trelloBoardId && !list.trelloListId) {
            const opt = document.createElement('div');
            opt.className = 'list-option-item';
            opt.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span>Trello Tracker 2</span>`;
            opt.onclick = (e) => {
                e.stopPropagation();
        ...
        if (activeBoard.trelloBoardId && !list.trelloListId) {
            const opt = document.createElement('div');
            opt.className = 'list-option-item';
            opt.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span>Trello Tracker 3</span>`;
            opt.onclick = (e) => {
                e.stopPropagation();
                openTrelloMappingGenerator(list, 'trello3');
                optionsMenu.style.display = 'none';
            };
            optionsMenu.appendChild(opt);
        }
        ...
        if (activeBoard.trelloBoardId) {
            ...
            if (list.trelloListId) {
                if (list.trackerType !== 'ads' && list.trackerType !== 'ads2' && list.trackerType !== 'trelloSpeech' && list.trackerType !== 'trello3') {
                    const t1Option = document.createElement('div');
                    t1Option.className = 'list-option-item';
                    t1Option.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg><span>Unlink Trello Tracker 1</span>`;
                    t1Option.onclick = (e) => {
                        e.stopPropagation();
                        if(confirm("Unlink Trello Tracker 1?")) {
                            list.trelloListId = null;
                            list.cards = list.cards.filter(c => !c.isTrello);
                            saveState();
                            render();
                    ...
                    optionsMenu.appendChild(t1Option);
                } else if (list.trackerType === 'trelloSpeech') {
                    const t2Option = document.createElement('div');
                    t2Option.className = 'list-option-item';
                    t2Option.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg><span>Unlink Trello Tracker 2</span>`;
                    t2Option.onclick = (e) => {
                        e.stopPropagation();
                        if(confirm("Unlink Trello Tracker 2?")) {
                            list.trelloListId = null;
                            list.trackerType = null;
                            list.cards = list.cards.filter(c => !c.isTrello);
                            saveState();
                    ...
                    optionsMenu.appendChild(t2Option);
                } else if (list.trackerType === 'trello3') {
                    const t3Option = document.createElement('div');
                    t3Option.className = 'list-option-item';
                    t3Option.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg><span>Unlink Trello Tracker 3</span>`;
                    t3Option.onclick = (e) => {
                        e.stopPropagation();
                        if(confirm("Unlink Trello Tracker 3?")) {
                            list.trelloListId = null;
                            list.trackerType = null;
                            list.cards = list.cards.filter(c => !c.isTrello);
                            saveState();
                    ...
                    optionsMenu.appendChild(t3Option);
                }
            }
            ...
            if(tasksOption) tasksOption.onclick = (e) => {
                ...
                if (list.trelloTasksListId) {
                    ...
                    window.pendingTrelloTasksIndex = 1;
                    openTrelloTasksMappingModal(list);
                }
                optionsMenu.style.display = 'none';
            };
            optionsMenu.appendChild(tasksOption);

            const isTasks2Unlink = !!list.trelloTasks2ListId;
            const tasks2Text = isTasks2Unlink ? 'Unlink Trello Tasks 2' : 'Link Trello Tasks 2';
            const tasks2Option = document.createElement('div');
            tasks2Option.className = 'list-option-item';
            tasks2Option.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${tasksIcon}</svg><span>${tasks2Text}</span>`;
            if(tasks2Option) tasks2Option.onclick = (e) => {
                e.stopPropagation();
                if (list.trelloTasks2ListId) {
                    if(confirm("Unlink Trello Tasks 2 list?")) {
                        list.trelloTasks2ListId = null;
                        list.trelloTasks2BoardId = null;
                        list.cards = list.cards.filter(c => !c.isTrelloTask2);
                        saveState();
                        render();
                    }
                } else {
                    window.pendingTrelloTasksIndex = 2;
                    openTrelloTasksMappingModal(list);
                }
                optionsMenu.style.display = 'none';
            };
            optionsMenu.appendChild(tasks2Option);
        }
        ...
        const cardsCheckOptionIcon = isShowCardsCheck
            ? `<path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24M1 1l22 22"></path>`
            : `<path d="M9 11l3 3L22 4"></path><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path>`;

        const cardsCheckOption = document.createElement('div');
        cardsCheckOption.className = 'list-option-item';
        cardsCheckOption.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${cardsCheckOptionIcon}</svg><span>${cardsCheckOptionText}</span>`;
        if (cardsCheckOption) cardsCheckOption.onclick = (e) => {
            e.stopPropagation();
            if (!activeBoard.showCardsCheck) activeBoard.showCardsCheck = {};
            activeBoard.showCardsCheck[list.id] = !activeBoard.showCardsCheck[list.id];
            saveState();
            render();
            optionsMenu.style.display = 'none';
        };
        ...
        edges.forEach(edge => {
            ...
            const hasTrelloTrackers = activeBoard.connections && activeBoard.connections.some(c => 
                c.source === list.id && c.sourcePort === edge && activeBoard.lists.find(l => l.id === c.target && l.trelloListId && l.trackerType !== 'ads' && l.trackerType !== 'ads2' && l.trackerType !== 'trelloSpeech' && l.trackerType !== 'trello3')
            );
            ...
            const hasTrello3Trackers = activeBoard.connections && activeBoard.connections.some(c => 
                c.source === list.id && c.sourcePort === edge && activeBoard.lists.find(l => l.id === c.target && l.trelloListId && l.trackerType === 'trello3')
            );
            ...
            const hasTrackersOnEdge = hasTrelloTrackers || hasTrelloSpeechTrackers || hasTrello3Trackers || hasAdsTrackers || hasAds2Trackers || hasPipedriveTrackers || hasClientHappinessTrackers || hasMoneySmellingTrackers || hasNewClientsTrackers;
            ...
            if (hasTrackersOnEdge) {
                ...
                if (list.collapsedEdges.includes(edge)) {
                    ...
                    if (hasTrello3Trackers) list.collapsedEdges.push(`${edge}:trello3`);
                }
                
                list.edgeOrder = list.edgeOrder || {};
                const userOrder = list.edgeOrder[edge] || ['clientHappiness', 'moneySmelling', 'newClients', 'pipedrive', 'trello', 'trello3', 'trelloSpeech', 'ads', 'ads2'];
                const edgeDict = {};
                ...
                if (hasAds2Trackers) {
                    const isAdsCollapsed = list.collapsedEdges.includes(`${edge}:ads2`);
                    const dropAttr = `ondragover="event.preventDefault();" ondrop="if(window.handleToggleReorder) window.handleToggleReorder(event, '${list.id}', '${edge}', 'ads');"`;
                    if (isAdsCollapsed) {
                        edgeDict['ads2'] = `<div ${dropAttr} draggable="true" ondragstart="event.stopPropagation(); event.dataTransfer.setData('application/x-transfer-ads2', '${list.id}'); event.dataTransfer.effectAllowed='move';" style="cursor:inherit; display:inline-flex; align-items:center; justify-content:center;">
                            <svg data-tracker-type="ads2" width="32" height="32" viewBox="0 0 24 24" style="filter: drop-shadow(0 4px 6px rgba(0,0,0,0.5)); transform: scale(1.1);">
                                <circle cx="12" cy="12" r="10" fill="#00838F" stroke="#004D40" stroke-width="1.5"/>
                                <path d="M7 15 L11 11 L14 14 L18 10" fill="none" stroke="#18FFFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                <polyline points="15 10 18 10 18 13" fill="none" stroke="#18FFFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                <circle cx="7" cy="15" r="1.5" fill="#FFFFFF"/>
                        ...
                        edgeDict['ads2'] = `<div ${dropAttr} draggable="true" ondragstart="event.stopPropagation(); event.dataTransfer.setData('application/x-transfer-ads2', '${list.id}'); event.dataTransfer.effectAllowed='move';" style="cursor:inherit; display:inline-flex; align-items:center; justify-content:center;">
                            <svg data-tracker-type="ads2" width="32" height="32" viewBox="0 0 24 24" style="filter: drop-shadow(0 2px 8px rgba(0, 229, 255, 0.7)); transform: scale(1.1);">
                                <circle cx="12" cy="12" r="11" fill="none" stroke="#00E5FF" stroke-width="1" stroke-dasharray="2 2" opacity="0.8"/>
                                <circle cx="12" cy="12" r="9" fill="#006064" stroke="#00251A" stroke-width="1.5"/>
                                <path d="M7 14 L11 10 L14 13 L18 8" fill="none" stroke="#18FFFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                <polyline points="15 8 18 8 18 11" fill="none" stroke="#18FFFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        let trelloTasksBtnsContainer = null;
        if (list.trelloTasksListId || list.trelloTasks2ListId) {
            ...
            const createToggleBtn = (taskIndex) => {
                ...
                const count = list.cards.filter(c => (isTask2 ? c.isTrelloTask2 : c.isTrelloTask) && !c.isPinned).length;
                const label = (list.trelloTasksListId && list.trelloTasks2ListId) ? `Tasks ${taskIndex}` : `Tasks`;
                
                window.expandedTrelloLists = window.expandedTrelloLists || new Set();
                const keyId = list.id + (isTask2 ? '_t2' : '_t1');
                ...
                showBtn.onclick = (e) => {
                    e.stopPropagation();
                    const wasExpanded = window.expandedTrelloLists.has(keyId);
                    
                    if (!wasExpanded) {
                        window.expandedTrelloLists.add(keyId);
                        const otherKeyId = list.id + (isTask2 ? '_t1' : '_t2');
                        window.expandedTrelloLists.delete(otherKeyId);
                    } else {
                        window.expandedTrelloLists.delete(keyId);
                    }
                    
                    window.highestZIndex = (window.highestZIndex || 1000) + 1;
                    listContainer.style.zIndex = window.highestZIndex;
                    
                    Array.from(trelloTasksBtnsContainer.children).forEach(btn => {
                        if (btn._updateInner) btn._updateInner();
                    });
                    
                    const task1Cards = cardListEl.querySelectorAll('.trello-task-card');
                    task1Cards.forEach(c => {
                        if (window.expandedTrelloLists.has(list.id + '_t1')) c.classList.remove('collapsed');
                        else c.classList.add('collapsed');
                    });
                    
                    const task2Cards = cardListEl.querySelectorAll('.trello-task2-card');
                    task2Cards.forEach(c => {
                        if (window.expandedTrelloLists.has(list.id + '_t2')) c.classList.remove('collapsed');
                        else c.classList.add('collapsed');
                    });
            ...
            if (list.trelloTasksListId) trelloTasksBtnsContainer.appendChild(createToggleBtn(1));
            if (list.trelloTasks2ListId) trelloTasksBtnsContainer.appendChild(createToggleBtn(2));
            
            const isAnyExpanded = window.expandedTrelloLists && (window.expandedTrelloLists.has(list.id + '_t1') || window.expandedTrelloLists.has(list.id + '_t2'));
            if (isAnyExpanded) listContainer.style.zIndex = Math.max(window.highestZIndex || 1000, 1000);
            else listContainer.style.zIndex = '10';
        }
        ...
        cardsToRender.forEach(card => {
            ...
            if (card.isTrelloTask) {
                ...
                if ((!window.expandedTrelloLists || !window.expandedTrelloLists.has(list.id + '_t1')) && !card.isPinned) {
                    cardEl.classList.add('collapsed');
                }
            } else if (card.isTrelloTask2) {
                cardEl.classList.add('trello-task2-card');
                if ((!window.expandedTrelloLists || !window.expandedTrelloLists.has(list.id + '_t2')) && !card.isPinned) {
                    cardEl.classList.add('collapsed');
                }
            }
            ...
            } else if (list.trackerType === 'ads' || list.trackerType === 'ads2') {
            ...
            if (activeBoard.showCardsCheck && activeBoard.showCardsCheck[list.id]) {
            ...
            if (waVal) {
                ...
                if (waNum.length > 5) {
                        ...
                        // Use native app URI scheme to skip the browser prompt
                        waLink.href = `whatsapp://send?phone=${waNum.startsWith('+') ? waNum.substring(1) : waNum}`;
            ...
            if (list.isNewClients) {
                const addedDateWrap = document.createElement('div');
                addedDateWrap.style.marginTop = '8px';
                addedDateWrap.style.paddingTop = '8px';
                addedDateWrap.style.borderTop = '1px solid rgba(9, 30, 66, 0.06)';
                addedDateWrap.style.fontSize = '11px';
                addedDateWrap.style.color = '#7A869A';
                addedDateWrap.style.textAlign = 'right';
                addedDateWrap.style.fontWeight = '500';
                addedDateWrap.style.display = 'flex';
                addedDateWrap.style.justifyContent = 'flex-end';
                ...
                addedDateWrap.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg> <span>Added on ${formattedDate} at ${formattedTime}</span>`;
                cardEl.appendChild(addedDateWrap);
            }
        ...
        if (list.trelloTasksListId || list.trelloTasks2ListId) ctaText = 'Add a task';
        ...
        if(addBtn) addBtn.onclick = () => {
            ...
            const isTrelloTask = !!list.trelloTasksListId || !!list.trelloTasks2ListId;
        ...
        const isTrelloTrackerNode = (list.trelloListId || list.trelloBoardId || ( (list.trelloTasksListId || list.trelloTasks2ListId) && (!list.title || list.title.toLowerCase() !== 'me') )) && list.trackerType !== 'ads' && list.trackerType !== 'ads2' && !list.isClientHappiness && !list.isMoneySmelling;
        ...
        if (list.cards.length > 0 && !list.trelloTasksListId && !list.trelloTasks2ListId && list.trackerType !== 'ads' && list.trackerType !== 'ads2' && !list.pipedriveStageId) {
            listContainer.insertBefore(hideCardsBtn, cardListEl);
        }
        
        if (typeof trelloTasksBtnsContainer !== 'undefined' && trelloTasksBtnsContainer) {
            listContainer.insertBefore(trelloTasksBtnsContainer, cardListEl);
        }
                    ...
                    if (isTrelloCard) {
                        ...
                        if (targetTrelloListId) {
                            ...
                            fetch(createUrl, { method: 'POST' }).then(res => res.json()).then(newCard => {
                                ...
                                } else if (toList.trelloTasks2ListId) {
                                    movedItem.isTrelloTask2 = true;
                                } else {
...
if(confirmAddBtn) confirmAddBtn.onclick = async () => {
    ...
    if (title) {
        ...
        if (activeBoard.type === 'kanban') {
            ...
            if (list.trelloTasksListId || list.trelloTasks2ListId) {
                const targetTrelloListId = list.trelloTasksListId || list.trelloTasks2ListId;
                const isTask2 = targetTrelloListId === list.trelloTasks2ListId;
                ...
                try {
                    const res = await fetch(`https://api.trello.com/1/cards?idList=${targetTrelloListId}&name=${encodeURIComponent(title)}&key=${trelloKey}&token=${trelloToken}`, {
                        method: 'POST'
                    });
                    ...
                    list.cards.push({
                        ...
                        color: 'default',
                        ...(isTask2 ? { isTrelloTask2: true } : { isTrelloTask: true })
                    });
                    
                    showToast("Task created in Trello!");
                    
                    window.expandedTrelloLists = window.expandedTrelloLists || new Set();
                    window.expandedTrelloLists.add(list.id + (isTask2 ? '_t2' : '_t1'));
...
async function syncTrello() {
    ...
    const mappedLists = curBoard.lists.filter(l => l.trelloListId || l.trelloTasksListId || l.trelloTasks2ListId);
    ...
    const mappedTasks2Ids = mappedLists.map(l => l.trelloTasks2ListId).filter(Boolean);
    ...
    mappedLists.forEach(l => {
        ...
        else if (l.trelloTasks2BoardId) uniqueBoardIds.add(l.trelloTasks2BoardId);
    ...
    try {
        ...
        for (let i = 0; i < dataPayloads.length; i += 2) {
            ...
            listsPayload.forEach((tList) => {
                curBoard.lists.forEach(l => {
                    if (l.trelloListId === tList.id || l.trelloTasksListId === tList.id || l.trelloTasks2ListId === tList.id) {
        ...
        const oldStateStr = JSON.stringify(curBoard.lists.map(l => l.cards.filter(c => c.isTrello || c.isTrelloTask || c.isTrelloTask2)));
        const colorMemory = {};
        ...
        curBoard.lists.forEach(l => {
            l.cards.forEach(c => {
                if (c.color && (c.isTrello || c.isTrelloTask || c.isTrelloTask2)) {
                    colorMemory[c.id] = c.color;
                }
                if (c.adsMetrics && (c.isTrello || c.isTrelloTask || c.isTrelloTask2)) {
                ...
                if (c.isTrelloTask2) {
                    existingTrelloTasks2.push({ card: JSON.parse(JSON.stringify(c)), listId: l.id });
                }
            });
            l.cards = l.cards.filter(c => !c.isTrello && !c.isTrelloTask && !c.isTrelloTask2);
        });
        ...
        allTrelloCards.forEach(tCard => {
            ...
            const isTasks2Matched = mappedTasks2Ids.includes(tCard.idList);
            
            if (!isTrackerMatched && !isTasksMatched && !isTasks2Matched) return;
        ...
        existingTrelloTasks2.forEach(taskObj => {
            if (!activeTrelloCardIds.has(taskObj.card.id)) {
                taskObj.card.isTrelloDeleted = true;
                const parentList = curBoard.lists.find(l => l.id === taskObj.listId);
                if (parentList) {
        ...
        const newStateStr = JSON.stringify(curBoard.lists.map(l => l.cards.filter(c => c.isTrello || c.isTrelloTask || c.isTrelloTask2)));
    ...
    if (curBoard.connections) {
        ...
        motherListIds.forEach(sourceId => {
            ...
            curBoard.connections.forEach(c => {
                if (c.source === sourceId) {
                    ...
                    if (targetList && targetList.cards) {
                        if (targetList.trelloListId && targetList.trackerType !== 'ads' && targetList.trackerType !== 'ads2' && targetList.trackerType !== 'trelloSpeech' && targetList.trackerType !== 'trello3') {
                            ...
                            Object.keys(eff).forEach(k => {
                                ...
                                if (pId !== targetList.id) {
                                    ...
                                    if (allD2.has(targetList.id)) {
                                        ...
                                        if (pType === 'trello' && (targetList.trelloTasksListId || targetList.trelloTasks2ListId || targetList.trelloBoardId || targetList.trelloListId) && targetList.trackerType !== 'ads' && targetList.trackerType !== 'ads2' && targetList.trackerType !== 'trelloSpeech' && targetList.trackerType !== 'trello3') {
                                            typeMatches = true;
                                        } else if (pType === 'trello3' && targetList.trackerType === 'trello3') {
                                            typeMatches = true;
                        ...
                        if (!listHiddenByFilter && targetList.cards && targetList.cards.length === 0 && !targetList.trelloTasksListId && !targetList.trelloTasks2ListId && (targetList.trelloBoardId || targetList.trelloListId) && targetList.trackerType !== 'ads' && targetList.trackerType !== 'ads2' && (!targetList.title || targetList.title.toLowerCase() !== 'me')) {
            ...
            const packTargets = (targets) => {
                ...
                Object.keys(byDirection).forEach(dir => {
                    ...
                    sortedLists.forEach((list, index) => {
                        const isAds = list.trackerType === 'ads' || list.trackerType === 'ads2';
                        const isTs = list.trackerType === 'trelloSpeech';
                        const isT3 = list.trackerType === 'trello3';
                        const offsetX = isAds ? (sourceList.adsOffsetX !== undefined ? sourceList.adsOffsetX : 0) : (isT3 ? (sourceList.trello3OffsetX !== undefined ? sourceList.trello3OffsetX : 800) : (isTs ? (sourceList.trelloSpeechOffsetX !== undefined ? sourceList.trelloSpeechOffsetX : 800) : (sourceList.trelloOffsetX !== undefined ? sourceList.trelloOffsetX : 0)));
                        const spacingY = isAds ? (sourceList.adsSpacingY !== undefined ? sourceList.adsSpacingY : 60) : (isT3 ? (sourceList.trello3SpacingY !== undefined ? sourceList.trello3SpacingY : 60) : (isTs ? (sourceList.trelloSpeechSpacingY !== undefined ? sourceList.trelloSpeechSpacingY : 60) : (sourceList.trelloSpacingY !== undefined ? sourceList.trelloSpacingY : 60)));
                        const baseTypeOffset = isAds ? (sourceList.adsOffsetY !== undefined ? sourceList.adsOffsetY : 0) : (isT3 ? (sourceList.trello3OffsetY !== undefined ? sourceList.trello3OffsetY : 0) : (isTs ? (sourceList.trelloSpeechOffsetY !== undefined ? sourceList.trelloSpeechOffsetY : 0) : (sourceList.trelloOffsetY !== undefined ? sourceList.trelloOffsetY : 0)));
            ...
            if (allTargets.length > 0) {
                const trelloTargets = allTargets.filter(t => t.list.trackerType !== 'trelloSpeech' && t.list.trackerType !== 'trello3' && t.list.trackerType !== 'ads' && t.list.trackerType !== 'ads2');
                const speechTargets = allTargets.filter(t => t.list.trackerType === 'trelloSpeech');
                const trello3Targets = allTargets.filter(t => t.list.trackerType === 'trello3');
                const adsTargets = allTargets.filter(t => t.list.trackerType === 'ads' || t.list.trackerType === 'ads2');

                if (trelloTargets.length > 0) packTargets(trelloTargets);
                if (speechTargets.length > 0) packTargets(speechTargets);
                if (trello3Targets.length > 0) packTargets(trello3Targets);
